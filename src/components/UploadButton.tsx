...
const UploadDropZone = () => {
  ...
  // keep polling db until we know pdf has been uploaded
  ...
  const startSimulatedProgress = () => {
    ...
    const interval = setInterval(() => {
      setUploadProgress((prevProgress) => {
        if (prevProgress >= 95) {
          clearInterval(interval); // stops it at this point
          return prevProgress;
        }
  ...
  return (
    <Dropzone
      ...
      onDrop={async (acceptedFile) => {
        // Does the progress bar increment
        setIsUploading(true);
...
const UploadButton = () => {
  ...
  return (
    <Dialog
      open={isOpen}
      onOpenChange={(v) => {
        // Decides when to open the dialog or not
        if (!v) {
          setIsOpen(v);
        }
      }}
